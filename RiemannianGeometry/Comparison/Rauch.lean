...
/-- Input-level `dexp` comparison exported from the repaired reduction layer: once the scalar
comparison theorem has been supplied for the reduced norm data, the public Rauch consequence is the
same `modelPosDomain` pointwise bound. -/
theorem dexp_normComparison_of_scalarComparison
    ...
    DexpNormComparison input.modelData.k exp radial direction input.data.modelNorm :=
  dexp_normComparison_of_rauch hjacobi
    (rauch_normComparison_of_scalarComparison input)
...
Given a `RauchInputData` for each standard basis direction and the identification of
each Jacobi norm with the corresponding `dexpNorm`, this produces the full
`BasiswiseDexpNormComparison` package.

This eliminates the need to construct the basiswise structure manually — once the
scalar comparison is available per direction, the packaging is automatic. -/
